...
pub fn migration_table_exists(conn_str: &str) -> Result<bool> {
    ...
    assert!(rows.len() == 1, "Migration table check: Expected 1 returned row");
    Ok(rows[0].tag == 1)
}
...
pub fn run_migration(conn_str: &str, filename: &Path) -> Result<()> {
    ...
    let mut conn = Conn::new(conn_str).chain_err(|| "Connection Error")?;
    conn.query(&buf).map_err(|e| format_err!(ErrorKind::Migration, "{}", e))?;
    Ok(())
}


#[cfg(not(feature="with-mysql"))]
pub fn run_migration_str(_conn_str: &str, _stmt: &str) -> Result<connection::markers::WithMySQLFeatureRequired> {
    panic!("\n** Migrant ERROR: `with-mysql` feature required **");
}
