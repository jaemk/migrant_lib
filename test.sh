...
MYSQL=mysql://__migrant_testing:pass@localhost/__migrant_testing


# Runs a mysql command as root, using a password if provided
# $1 -> command to execute
# $2 -> optional mysql root password
function do_mysql() {
    if [[ -z $2 ]]; then
        mysql -e "$1"
    else
...
function setup() {
    ...
    do_mysql "create user '__migrant_testing'@'localhost' identified by 'pass';" $1
    do_mysql "create database __migrant_testing;" $1
    do_mysql "grant all privileges on __migrant_testing.* to '__migrant_testing'@'localhost';" $1
}
...
function teardown() {
    ...
    do_mysql "drop user '__migrant_testing'@'localhost';" $1
